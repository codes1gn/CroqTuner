...
|---|---|---|
| `macro` | SPMM_WARP_N, SPMM_STAGES, OUTPUT_PAD, L2 promotion, META_TILE_COLS | Early (1-8 from_best, 1-15 from_scratch) |
| `structural` | CTA scheduling, SMEM layout, launch_bounds, barrier depths, output epilogue, prefetch hints, fence patterns | Mid (9-18 from_best, 15-60 from_scratch) |
| `choreo` | New .co kernel, different choreo flags, tile size changes in EDSL, warp topology changes | Mid-late (19-25 from_best, 40-100 from_scratch) |
| `ncu_micro` | Inline PTX, bank conflict fixes, nanosleep, mbarrier tuning, fence_proxy_async, loop unroll, regcount | Late (26-30 from_best, 80-150 from_scratch) |
...
|---|---|
| >= 3 | Run ncu (even in from_best mode), switch category |
...
| >= 15 | Re-read reference kernel optimizations, try porting a winning technique |

### Rule D5: Phase Progression
For from_scratch mode (150 iters), you MUST progress through optimization phases:
- By iter 30: at least 5 ideas should be `structural` (not just `macro`)
- By iter 80: at least 3 ideas should be `choreo`
- By iter 120: at least 3 ideas should be `ncu_micro`

For from_current_best mode (30 iters), compressed:
- By iter 10: at least 2 ideas should be `structural`
- By iter 20: at least 1 idea should be `choreo`
- By iter 25: at least 1 idea should be `ncu_micro`
